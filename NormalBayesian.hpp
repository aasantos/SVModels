#ifndef NormalBayesian_hpp
#define NormalBayesian_hpp
...
public:
    ...
    NormalBayesian(double *x,int n,double mu,double sigma,Random *random)
    {
        this->n = n;
        this->mu = mu;
        this->sigma = sigma;
    ...
    void simulateparameters()
    {
        if(this->updatemu){
            if(this->mudiffuse){
                this->mu = Stats(x,this->n).mean() + (this->sigma/sqrt((double)this->n))*random->normal();
            }else{
                if(this->mupriortype == 0){
                    double meansample = Stats(x,this->n).mean();
                    double varsample = this->sigma*this->sigma/(double)this->n;
                    ...
                    double meansample = Stats(x,this->n).mean();
                    double varsample = this->sigma*this->sigma/(double)this->n;
                    this->mu = normalbetasimul(this->mu,meansample,sqrt(varsample), this->mubetaprior[0], this->mubetaprior[1]);
                }
            }
        }
        if(this->updatesigma){
            ...
            for(int i=0;i<this->n;i++){
                rss += (this->x[i] - this->mu)*(this->x[i] - this->mu);
            }
            if(sigmadiffuse){
